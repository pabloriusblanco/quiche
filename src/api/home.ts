...
export async function getMonthRecipes(): Promise<PostResponse[]> {
  const response = await api.get(`Home/TopRatedPostByMonth`);
  return response.data.data.posts;
}

export async function getFeaturedDailyPost(): Promise<PostResponse[]> {
  const response = await api.get(`Home/TopRatedPostByDay`);
  return response.data.data.posts;
}
